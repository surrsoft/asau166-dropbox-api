// https://github.com/googleapis/google-api-nodejs-client/blob/main/src/apis/sheets/v4.ts
...
export interface Schema$CellData {
   ...
   *
   * Значение, которое пользователь ввел в ячейку. Например, `1234`, `'Hello'`, или `=NOW()` Примечание: Даты,
   * времена и DateTimes представлены как doubles-числа в формате порядкового номера.
   */
...
export interface Schema$ExtendedValue {
  ...
  /**
   * Represents a string value. Leading single quotes are not included. For example, if the user typed `'123` into the
   * UI, this would be represented as a `stringValue` of `"123"`.
   *
   * Представляет строковое значение. Ведущие одинарные кавычки не учитываются. Например, если пользователь
   * набрал в пользовательском интерфейсе `'123`, это будет представлено как `stringValue` `"123"`.
   */
...
export interface Schema$UpdateCellsRequest {
   ...
   * CellData; 'row.values.' should not be specified. A single `"*"` can be used as short-hand for listing every field.
   *
   * Поля CellData, которые должны быть обновлены. Должно быть указано хотя бы одно поле. Корнем является
   * CellData; 'row.values.' не должно быть указано. В качестве сокращения для перечисления каждого поля можно
